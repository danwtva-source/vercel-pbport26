        ...
        <div className="relative w-full max-w-3xl mx-auto">
            <div className="overflow-hidden rounded-3xl shadow-2xl bg-white border border-gray-100 relative h-[520px]">
                ...
                <div className="absolute bottom-6 left-0 right-0 flex justify-center gap-2">
                    {slides.map((_, i) => (
                        <button key={i} onClick={() => setCurrentIndex(i)} className={`w-3 h-3 rounded-full transition-all ${i === currentIndex ? 'bg-brand-purple w-8' : 'bg-gray-300 hover:bg-gray-400'}`} />
                    ))}
                </div>
            </div>
        </div>
  ...
  return (
    <div className="animate-fade-in bg-gradient-to-br from-purple-100 via-purple-200 to-teal-100 min-h-[calc(100vh-80px)]">
      ...
      <section className="max-w-4xl mx-auto px-4 pb-20 text-center">
          ...
          <p className="text-gray-700 font-arial text-lg leading-relaxed">Participatory Budgeting (PB) puts decision-making power in the hands of residents. You can help choose which local projects receive funding by voting for your favourites.</p>
      </section>
    </div>
  );
};
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-purple-100 via-purple-200 to-teal-100 py-12 px-4 animate-fade-in">
            <div className="max-w-5xl mx-auto">
                <div className="text-center mb-10"><h1 className="text-4xl font-bold font-dynapuff text-brand-purple mb-4">Community Priorities</h1></div>
                <div className="flex justify-center mb-8 border-b-2 border-gray-200/50">
                    {[ { id: 'blaenavon', label: 'Blaenavon' }, { id: 'thornhill', label: 'Thornhill & Upper Cwmbran' }, { id: 'trevethin', label: 'Trevethin, Penygarn & St. Cadocs' } ].map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`px-4 md:px-8 py-4 font-bold font-dynapuff transition-all ${activeTab === tab.id ? 'bg-white/80 backdrop-blur text-brand-purple border-b-4 border-brand-purple rounded-t-xl' : 'text-gray-600 hover:bg-white/40 rounded-t-xl'}`}>{tab.label}</button>
                    ))}
                </div>
                    ...
                    <div className="grid md:grid-cols-12 gap-12">
                            ...
                            <div className="h-64 w-full bg-gray-50 rounded-xl border border-gray-100 flex items-end justify-around p-4">
                                {data.data.map((item, i) => (
                                    <div key={i} className="w-8 bg-brand-purple/80 rounded-t-md relative group hover:bg-brand-teal transition-colors" style={{ height: `${(item.value / 150) * 100}%` }}>
                                        <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 text-xs font-bold bg-gray-800 text-white px-2 py-1 rounded opacity-0 group-hover:opacity-100 whitespace-nowrap z-10">{item.label}: {item.value}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
                        <div className="md:col-span-7">
                            <div className="grid gap-4">
                                {data.data.map((item, i) => (
                                    <div key={i} className="bg-gradient-to-r from-white to-gray-50 border border-purple-100 p-4 rounded-xl shadow-sm flex items-center">
                                        <span className="text-2xl font-bold font-dynapuff text-brand-purple mr-4">#{i+1}</span>
                                        <div><h4 className="font-bold text-gray-800 font-dynapuff">{item.label}</h4><p className="text-sm text-gray-500 font-arial">Identified by community consensus.</p></div>
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-purple-100 via-purple-200 to-teal-100 py-16 px-4 animate-fade-in">
            <div className="max-w-5xl mx-auto bg-white/90 backdrop-blur rounded-3xl shadow-2xl p-8 md:p-12">
                <div className="text-center mb-12"><h1 className="text-4xl font-bold font-dynapuff text-brand-purple mb-2">PB Initiative Timeline</h1></div>
                <div className="space-y-12 relative">
                    ...
                    {events.map((e, i) => (
                        <div key={i} className={`flex flex-col md:flex-row items-center ${i % 2 === 0 ? 'md:flex-row-reverse' : ''} group`}>
                            ...
                            <div className={`z-10 flex items-center justify-center w-14 h-14 border-4 rounded-full shadow-xl shrink-0 transition-transform group-hover:scale-110 ${e.status === 'done' ? 'bg-brand-purple text-white border-brand-purple' : 'bg-white text-gray-300'}`}>{e.status === 'done' ? '✓' : i + 1}</div>
                            <div className="flex-1 md:w-1/2 p-4 text-center md:text-left">
                                <div className="bg-white p-6 rounded-2xl shadow-lg border-b-4 border-brand-purple"><span className="inline-block px-3 py-1 rounded-full text-xs font-bold mb-2 uppercase tracking-wider bg-purple-100 text-purple-700">{e.date}</span><h3 className="text-xl font-bold text-gray-800 font-dynapuff mb-1">{e.title}</h3><p className="text-gray-600 text-sm font-arial">{e.desc}</p></div>
                            </div>
                        </div>
                    ))}
                </div>
    ...
    return (
        <div className="min-h-screen bg-slate-50 py-12 px-4 animate-fade-in">
            <div className="max-w-4xl mx-auto">
                <div className="text-center mb-10"><h1 className="text-4xl font-bold font-dynapuff text-brand-purple mb-4">Guidance & Forms</h1></div>
                    ...
                    {COMMITTEE_DOCS.map((doc, i) => (
                        <Card key={i} onClick={() => window.open(doc.url, '_blank')} className="cursor-pointer hover:border-brand-purple hover:bg-purple-50 group">
                            <div className="flex items-start gap-4">
                                <div className="p-3 bg-purple-100 text-purple-600 rounded-xl group-hover:bg-purple-200 transition-colors">📄</div>
                                <div><h3 className="font-bold text-lg text-gray-800 mb-1">{doc.title}</h3><p className="text-sm text-gray-500 mb-2">{doc.desc}</p></div>
                            </div>
                        </Card>
                    ))}
                </div>
