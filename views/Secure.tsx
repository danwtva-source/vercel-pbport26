...
export const AdminDashboard: React.FC<{ onNavigatePublic: (v:string)=>void, onNavigateScoring: ()=>void }> = ({ onNavigatePublic, onNavigateScoring }) => {
    ...
    useEffect(() => {
        api.getUsers().then(u => { setUsers(u); const me = u.find(x => x.role === 'admin'); if(me) setCurrentUser(me); }).catch(e => console.error("Error loading users:", e));
        api.getApplications('All').then(setApps).catch(e => console.error("Error loading applications:", e));
        api.getPortalSettings().then(setSettings).catch(e => console.error("Error loading settings:", e));
    }, []);
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            {activeTab === 'applications' && <Card><div className="flex justify-between mb-4"><h3 className="font-bold text-xl">All Applications</h3><Button size="sm" onClick={() => exportToCSV(apps, 'all_apps')}>Export CSV</Button></div><div className="space-y-2">{apps.length === 0 ? <p className="text-gray-500 py-8 text-center">No applications yet.</p> : apps.map(a => <div key={a.id} className="flex justify-between items-center p-3 border rounded-lg hover:bg-gray-50"><div><div className="font-bold">{a.projectTitle}</div><div className="text-xs text-gray-500">{a.orgName} • {a.area}</div></div><Badge>{a.status}</Badge></div>)}</div></Card>}
